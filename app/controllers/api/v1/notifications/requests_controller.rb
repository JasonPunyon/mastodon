...
class Api::V1::Notifications::RequestsController < Api::BaseController
  ...
  before_action -> { doorkeeper_authorize! :write, :'write:notifications' }, only: :accept
  ...
  def index
    with_read_replica do
      ...
      @relationships = StatusRelationshipsPresenter.new(@requests.map(&:last_status), current_user&.account_id)
    end
  ...
  def accept
    NotificationPermission.create!(account: current_account, from_account: @request.from_account)
    render_empty
  end
  ...
  def load_requests
    requests = NotificationRequest.where(account: current_account, dismissed: false).includes(:last_status, from_account: [:account_stat, :user]).to_a_paginated_by_id(
  ...
  def pagination_params(core_params)
    params.slice.permit.merge(core_params)
  end
end
